...
namespace Slate{
	...
	public class DirectorCamera : MonoBehaviour, IDirectableCamera {
		...
		private bool _matchMainWhenActive   = false;
		[SerializeField] [HideInInspector]
		private bool _setMainWhenActive     = false;
		[SerializeField] [HideInInspector]
		private bool _autoHandleActiveState = false;
		[SerializeField] [HideInInspector]
		...
		public static DirectorCamera current{
			get
			{
				if (_current == null){
						...
						if (_current == null){
							_current = new GameObject("DirectorCameraRoot").AddComponent<DirectorCamera>();
		...
		Camera CreateRenderCamera(){
            ...
            _cam.transform.gameObject.SetActive(false);
        ...
        public static Camera CurrentCamera(string name)
        {
           Camera cam = null;
           if (autoHandleActiveState){
				cam = renderCamera;
		...
		void OnDrawGizmos(){
            ...
            Gizmos.DrawCube(transform.position, new Vector3(0.2f, 0.05f, 0.2f));

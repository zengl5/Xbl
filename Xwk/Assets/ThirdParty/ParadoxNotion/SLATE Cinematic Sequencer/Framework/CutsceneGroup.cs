...
namespace Slate{
	...
	abstract public class CutsceneGroup : MonoBehaviour, IDirectable {
        ...
        /// <summary>
        /// 删除originalActor，支持动态生成actor 黄志龙 2018-7-26
        /// </summary>

        //Initialize actor reference mode
        void InstantiateLocalActor(){
            originalActor = actor;
            actor = (GameObject)Instantiate(originalActor);
            ...
            actor.transform.SetParent(actor.transform.parent, false);
            //添加动态阴影
        //    ProjectorShadow.Instance.SetShadow(actor);
            
#if UNITY_EDITOR 
            //not really needed, but avoids duplicate instaces when user undo immediately after an initialize.

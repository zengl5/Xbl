...
namespace Slate{
	...
	public interface IDirectableTimePointer{
        ...
        void Reback(float currentTime, float previousTime);
	} 
	...
	public struct StartTimePointer : IDirectableTimePointer{
        ...
        void IDirectableTimePointer.Reback(float currentTime, float previousTime)
        {
            if (currentTime <= target.startTime || currentTime <= 0)
            {
                triggered = false;
            }
            else if (target.startTime < currentTime)
            {
                triggered = true;
            }
        }

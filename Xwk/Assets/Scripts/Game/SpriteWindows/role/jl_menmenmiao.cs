...
public class jl_menmenmiao : baseRoleInfo
{
    ...
    void Awake()
    {
        base.Awake();
        AddBoxCollider();
        animLength = new[] { 2.4f, 2f, 3.3f, 3.3f };
    ...
    public override void OnClickRoleEvent()
    {
        if (!SpriteIconMgr.Instance.GetSpriteData().lockIcon)//还没有解锁，跳转到指定场景
        {
            ...
            YB.XWK.MainScene.LocalData.m_SpiritType = WizardItemName.Wizard_menmenmiao;
            YBSceneLoadingMgr.Instance.LoadMulitSceneAsync("wk_scene_01", "Piercingeye", () => { Utility.SetMainScene("Piercingeye"); });
        }
        else
        {
            if (anim.GetCurrentAnimatorClipInfo(0).Length > 0)
                if (!anim.GetCurrentAnimatorClipInfo(0)[0].clip.name.Equals("jl_00005_show01#anim"))
                {
                    anim.Play("show01#anim");
                    Refresh();
                }
            Spwindow.PlayCharacterAudio("xwk_jlej_46");
        }
    }

...
using UnityEngine;
public class jl_xixilang : baseRoleInfo
{
    void Awake()
    {
        base.Awake();
        AddBoxCollider();
        animLength = new[] { 4.33f, 2f, 3.33f, 3.3f };
    ...
    void AddBoxCollider()
    {
        ...
        box.size = new Vector3(60, 80, 100);
        box.tag = "meshButton";
    }
    ...
    public override void OnClickRoleEvent()
    {
        if (!SpriteIconMgr.Instance.GetSpriteData().lockIcon)//还没有解锁，跳转到指定场景
        {
            ...
            YB.XWK.MainScene.LocalData.m_SpiritType = WizardItemName.Wizard_xixilang;
            YBSceneLoadingMgr.Instance.LoadMulitSceneAsync("wk_scene_01", "Piercingeye", () => { Utility.SetMainScene("Piercingeye"); });
        }
        else
        {
            if (anim.GetCurrentAnimatorClipInfo(0).Length > 0)
                if (!anim.GetCurrentAnimatorClipInfo(0)[0].clip.name.Equals("jl_00010_show01#anim"))
                {
                    anim.Play("show01#anim");
                    Refresh();
                }
            Spwindow.PlayCharacterAudio("xwk_jlej_50");
        }
    }

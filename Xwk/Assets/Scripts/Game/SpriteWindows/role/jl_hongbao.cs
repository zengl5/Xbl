...
public class jl_hongbao : baseRoleInfo
{
    void Awake()
    {
        base.Awake();
        AddBoxCollider();
        animLength = new[] { 6.63f, 2f, 4.6f, 5.8f };
    ...
    public override void OnClickRoleEvent()
    {
        if (!SpriteIconMgr.Instance.GetSpriteData().lockIcon)//还没有解锁，跳转到指定场景
        {
            ...
            YB.XWK.MainScene.LocalData.m_SpiritType = WizardItemName.Wizard_hongbao;
            YBSceneLoadingMgr.Instance.LoadMulitSceneAsync("wk_scene_01", "Piercingeye", () => { Utility.SetMainScene("Piercingeye"); });
        }
        else
        {
            if (anim.GetCurrentAnimatorClipInfo(0).Length > 0)
                if (!anim.GetCurrentAnimatorClipInfo(0)[0].clip.name.Equals("jl_00015_show01#anim"))
                {
            ...
            if (id == 0)
            {
                Spwindow.PlayCharacterAudio("xwk_jlej_38");
            }
            else if (id == 1)
            {
                Spwindow.PlayCharacterAudio("xwk_jlej_39");
            }
            else if (id == 2)
            {
                Spwindow.PlayCharacterAudio("xwk_jlej_40");
            }
            else if (id == 3)
            {
                Spwindow.PlayCharacterAudio("xwk_jlej_41");
            }
        }
    }

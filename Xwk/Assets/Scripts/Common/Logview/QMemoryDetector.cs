...
namespace QFramework
{
    ...
    public class QMemoryDetector
    {
        ...
        void OnGUI()
        {
            ...
            GUI.Label(allocMemoryRect, string.Format(TotalAllocMemroyFormation, Profiler.GetTotalAllocatedMemoryLong() * ByteToM), QConsole.ConsoleFontGUIStyle);
            GUI.Label(reservedMemoryRect, string.Format(TotalReservedMemoryFormation, Profiler.GetTotalReservedMemoryLong() * ByteToM), QConsole.ConsoleFontGUIStyle);
            GUI.Label(unusedReservedMemoryRect, string.Format(TotalUnusedReservedMemoryFormation, Profiler.GetTotalUnusedReservedMemoryLong() * ByteToM), QConsole.ConsoleFontGUIStyle);
            GUI.Label(monoHeapRect, string.Format(MonoHeapFormation, Profiler.GetMonoHeapSizeLong() * ByteToM), QConsole.ConsoleFontGUIStyle);
            GUI.Label(monoUsedRect, string.Format(MonoUsedFormation, Profiler.GetMonoUsedSizeLong() * ByteToM), QConsole.ConsoleFontGUIStyle);
        }
    }

...
public class WavUtility
{
    ...
    public static AudioClip ToAudioClip(string filePath)
    {
        ...
        //fileBytes = PinYinREAD.ByteGZipCompress.Decompress(fileBytes);
    ...
    #region ---自定义保存audioClip路径---4/22---zack---
    public static bool SaveAudioClip(AudioClip audioClip,string filePath)
    {
        if(string.IsNullOrEmpty(filePath) || !filePath.StartsWith(Application.persistentDataPath))
        {
            Debug.Log("save audioClip fail with error path");
            return false;
        }
        if(audioClip == null)
        {
            Debug.Log("the clip u want save is null");
            return false;
        }
        try
        {
            ...
            WriteFileHeader(ref ms, fileSize);
            // file header (fmt)
            WriteFileFormat(ref ms, audioClip.channels, audioClip.frequency, bitDepth);
            // data chunks (data)
            WriteFileData(ref ms, audioClip, bitDepth);

            //byte[] bytes = PinYinREAD.ByteGZipCompress.Compress(ms.ToArray());

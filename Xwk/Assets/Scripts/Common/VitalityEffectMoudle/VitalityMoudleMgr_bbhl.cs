...
namespace XWK.VITALITY.MOUDLE {
    public class VitalityMoudleMgr_bbhl : MonoBehaviour
    {
        private GameObject _HuLuSpiritRole;
        private GameObject _XwkRole;
        private GameObject _HuLuHeadEffect;
        private GameObject _XwkBodyEffect;
        private GameObject _PowerEffect;
        private Vector3 _HuLuRolePos;
        ...
        private Animator _HuLuAnimator;
      ...
      //  private string _PressUIName = "UI_effect_CA";
        ...
        void Start()
        {
            ...
            PauseUIMoudleMgr.Instance.mGameGoMainCityAction += QuitGame;
            PlayTaskAudio();
            // C_UIMgr.Instance.OpenUI(_PressUIName, new Vector3(942.7f, 5.7f, 0.0f),6f); 
        ...
        {
            AudioManager.Instance.PlayerSound("story/bbhl/sound/bbhl_050.ogg", false,()=> {
        ...
        {
            _HuLuSpiritRole = GameResMgr.Instance.LoadResource<GameObject>("public/mesh/jl_00001/public_model_jl00001_1#mesh.prefab", true);
            _HuLuRolePos= _HuLuSpiritRole.transform.position = new Vector3(-12.4f,0.01f,2.8f);
            _HuLuSpiritRole.transform.localRotation = Quaternion.Euler(new Vector3(0f, 32f, 0f));
            _HuLuSpiritRole.gameObject.SetActive(true);

            _XwkRole = GameResMgr.Instance.LoadResource<GameObject>("public/mesh/wukong/public_model_wukong#mesh.prefab", true);
            _XwkRolePos = _XwkRole.transform.position = new Vector3(-11f,0,2f);
            _XwkRole.transform.localRotation = Quaternion.Euler(new Vector3(0f,-37f,0f));
            ...
            _HuLuHeadEffect = GameResMgr.Instance.LoadResource<GameObject>("public/Hero_Effect/prefab/public_effect_nlq_small.prefab", true);
            _HuLuHeadEffect.transform.position = new Vector3(_HuLuRolePos.x, _HuLuRolePos.y+1.7f, _HuLuRolePos.z);
            GameObject BYEffectHeadClone = new GameObject("public_effect_nlq_small_collider");
            BYEffectHeadClone.transform.position = _HuLuHeadEffect.transform.position;
           ...
           _BYEffectBoxcollider.center = new Vector3(0, 1f, 0);
            ...
            _PowerEffect.transform.position = _HuLuHeadEffect.transform.position;
            ...
            _CurrentCamera.transform.position = new Vector3(-10.9f,0.57f,6f);
            _CurrentCamera.transform.rotation = Quaternion.Euler(new Vector3(-6f,193f,0f));
            ...
            _HuLuAnimator = _HuLuSpiritRole.GetAddComponent<Animator>();
            _HuLuAnimator.runtimeAnimatorController = GameResMgr.Instance.LoadResource<RuntimeAnimatorController>("story/bbhl/animatorcontroller/bbhl_jl00001_2_cam14#Animator Controller");
            _GuideHand = GameResMgr.Instance.LoadResource<GameObject>("public/Hero_Effect/prefab/public_effect_shoudianji.prefab", true);
            _GuideHand.transform.position = new Vector3(-12.11f, 1.5f, 3.24f);
        ...
        protected void ClickPowerEffect()
        {
            ...
            if (_Progress < 1)
            {
                ...
                _XwkRole.transform.localRotation = Quaternion.Euler(0f,-37f+(7f+37f)*_Progress,0);
                _HuLuHeadEffect.transform.localScale = new Vector3(1 - _Progress, 1 - _Progress, 1 - _Progress);
                _XwkBodyEffect.transform.localScale = new Vector3(40+(_Progress * 100)/2, 40 + (_Progress * 100) / 2, 40 + (_Progress * 100) / 2);
                _CurrentCamera.transform.position = new Vector3(-11f+(-10f+11f)*_Progress, 0.57f, 6f+(8f-6f)*_Progress);
                _CurrentCamera.transform.localRotation = Quaternion.Euler(-6 + (-13 + 6) * _Progress, 193f, 0);
        ...
        public void QuitGame()
        {
            ...
            if (_HuLuSpiritRole != null)
            {
                GameObject.DestroyImmediate(_HuLuSpiritRole);
				_HuLuSpiritRole=null;
            }
            ...
            if (_HuLuHeadEffect != null)
            {
                GameObject.DestroyImmediate(_HuLuHeadEffect.gameObject);
            }
            if (_XwkRole != null)
            {
